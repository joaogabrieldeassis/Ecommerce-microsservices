...
public class CreateCartCommandHandler(INotifier notifier,
                                      CartContext context,
                                      IHttpContextAccessor httpContext) : CommandHandlerBase(notifier, httpContext), IRequestHandler<CreateCartCommand>
{
    ...
    {
        var cart = new Domain.AggregatesModel.CartAggregate.Cart(GetUserId());
